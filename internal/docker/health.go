...
func (hm *HealthMonitor) CheckHealth() ([]ServiceHealth, error) {
	...
	serviceMap := make(map[string][]Container)
	for _, container := range containers {
		serviceMap[container.Service] = append(serviceMap[container.Service], container)
	}
...
func (hm *HealthMonitor) checkContainerHealth(container Container) HealthCheck {
	check := HealthCheck{
	...
	if container.State != "running" {
		...
		check.Error = fmt.Sprintf("Container is %s", container.State)
		return check
	}
